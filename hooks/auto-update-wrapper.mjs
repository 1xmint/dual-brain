...
const install = spawnSync('npm', ['install', '-g', 'dual-brain@latest', '--quiet'], {
  stdio: 'ignore',
  timeout: 120000,
});

if (install.status === 0) {
  // Refresh hooks in the project workspace
  const rootResult = spawnSync('npm', ['root', '-g'], { encoding: 'utf8', timeout: 5000 });
  const npmRoot = (rootResult.stdout || '').trim();
  if (npmRoot) {
    spawnSync(process.execPath, [npmRoot + '/dual-brain/install.mjs', '--quiet'], {
      stdio: 'ignore',
      timeout: 60000,
    });
  }
