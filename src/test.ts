...
function run(args: string[], opts: Record<string, unknown> = {}): Promise<{ code: number | null; stdout: string; stderr: string }> {
...
describe('profile', () => {
  ...
  it('getAvailableProviders returns only enabled providers', () => {
    ...
    const providers = getAvailableProviders(profile as unknown as import('./profile.js').Profile);
  ...
  it('isSoloBrain returns true with one provider', () => {
    ...
    assert.equal(isSoloBrain(profile as unknown as import('./profile.js').Profile), true);
  });
  ...
  it('isSoloBrain returns false with two providers', () => {
    ...
    assert.equal(isSoloBrain(profile as unknown as import('./profile.js').Profile), false);
  });
  ...
  it('getHeadModel returns sonnet for solo-claude', () => {
    ...
    assert.equal(getHeadModel(profile as unknown as import('./profile.js').Profile), 'sonnet');
  });
  ...
  it('getHeadModel returns gpt-4o for solo-openai', () => {
    ...
    assert.equal(getHeadModel(profile as unknown as import('./profile.js').Profile), 'gpt-4o');
  });
  ...
  it('getHeadModel returns sonnet for dual profile (claude is default highest when ranks tie)', () => {
    ...
    const model = getHeadModel(profile as unknown as import('./profile.js').Profile);
...
describe('detect', () => {
  ...
  describe('detectTask full pipeline', () => {
    it('simple edit → {intent:edit, risk:low, complexity:simple, tier:execute}', () => {
      ...
      assert.ok(['low', 'medium'].includes(result.risk as string));
    ...
    it('search: "find where logger is used" → intent:search, tier:search or execute', () => {
      ...
      assert.ok(['search', 'execute'].includes(result.tier as string));
    });
...
describe('decide', () => {
  ...
  describe('decideRoute', () => {
    it('solo-claude $20 → haiku or sonnet, never opus', () => {
      ...
      assert.ok(['haiku', 'sonnet'].includes(decision.model as string), `Got: ${decision.model}`);
    ...
    it('dual profile, search task → picks a provider and a model', () => {
      ...
      assert.ok(['claude', 'openai'].includes(decision.provider as string));
  ...
  describe('getAvailableModels', () => {
    ...
    it('all openai models available (no subscription gating)', () => {
      ...
      const { openai } = getAvailableModels(profile as unknown as import('./profile.js').Profile);
...
describe('preference routing', () => {
  describe('parsePreferences — signal extraction', () => {
    ...
    it('null preferences → all nulls/false', () => {
      const signals = parsePreferences(null as unknown as undefined);
      assert.equal(signals.biasOverride,    null);
      assert.equal(signals.preferProvider,  null);
      assert.equal(signals.avoidProvider,   null);
      assert.equal(signals.alwaysDualBrain, false);
  ...
  describe('parsePreferences → decideRoute wiring', () => {
    ...
    it('cost-saver preference overrides balanced bias → cheaper model selected', () => {
      ...
      assert.ok(cheapModels.includes(decision.model as string), `Expected cheap model, got: ${decision.model}`);
    });
...
describe('dispatch', () => {
  ...
  describe('compressResult', () => {
    ...
    it('returns (no output) for null/undefined', () => {
      assert.equal(compressResult(null as unknown as undefined), '(no output)');
...
describe('dispatch safety features', () => {
  ...
  describe('validateDispatch', () => {
    it('returns _error when no CLI is available', () => {
      const rt = { claudeAvailable: false, codexAvailable: false, runtime: 'none' as const };
    ...
    it('falls back to openai when claude is unavailable but codex is', () => {
      const rt = { claudeAvailable: false, codexAvailable: true, runtime: 'codex-cli' as const };
      const result = validateDispatch({ provider: 'claude', model: 'sonnet', tier: 'execute' }, rt);
    ...
    it('falls back to claude when openai is unavailable but claude is', () => {
      const rt = { claudeAvailable: true, codexAvailable: false, runtime: 'claude-code' as const };
    ...
    it('keeps original decision when both CLIs available and model is valid', () => {
      const rt = { claudeAvailable: true, codexAvailable: true, runtime: 'claude-code' as const };
      const result = validateDispatch({ provider: 'claude', model: 'sonnet', tier: 'execute' }, rt);
  ...
  describe('checkWorktreeClean', () => {
    ...
    it('returns safe:true when owns is undefined', async () => {
      const result = await checkWorktreeClean(undefined as unknown as string[], process.cwd());
      assert.deepEqual(result, { safe: true });
    });
...
describe('integration: full pipeline', () => {
  ...
  it('simple edit routes to sonnet and dispatches', () => {
    ...
    assert.ok(['low', 'medium'].includes(detection.risk as string), `Unexpected risk: ${detection.risk}`);
    ...
    assert.ok(['sonnet', 'haiku'].includes(decision.model as string),
  ...
  it('cost-saver bias downgrades model', () => {
    ...
    assert.ok(['haiku', 'sonnet'].includes(decision.model as string),
