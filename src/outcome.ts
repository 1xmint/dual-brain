...
interface OutcomeStats {
  ...
  avgDuration: number;
  challengerHelpRate: number;
  topLessons: string[];
  modelBreakdown: Record<string, { count: number; successRate: number }>;
}
...
function outcomesDir(cwd: string): string {
  return join(cwd, '.dualbrain', 'outcomes');
}

function todayFile(cwd: string): string {
...
function readOutcomeFile(filePath: string): OutcomeRecord[] {
  try {
    return readFileSync(filePath, 'utf8')
      .split('\n')
      .filter(Boolean)
      .flatMap(line => {
        try { return [JSON.parse(line) as OutcomeRecord]; } catch { return []; }
      });
  } catch {
    return [];
  }
...
function last7DaysFiles(cwd: string): string[] {
...
function deriveIntent(prompt: string | undefined, tier: string | undefined): string {
...
function pruneOutcomes(cwd: string): void {
...
export function recordDispatchOutcome(dispatchInput: DispatchInput, result: DispatchResult): OutcomeRecord | null {
  try {
    ...
    try {
      import('./signal.js').then(({ scoreOutcome }) =>
        import('./routing-advisor.js').then(({ recordReward }) => {
          const scored = scoreOutcome(record as any);
...
export function computeRoutingScore(plan: Plan, result: Result & { duration?: number }, verification: Verification): number {
  let score = 3;
  if (result.success && (result.duration ?? 0) < 60_000) score += 1;
  if (verification.filesVerified && verification.testsPassed === true) score += 1;
  if (result.error) score -= 1;
  if ((result.duration ?? 0) > 180_000) score -= 1;
  if ((plan.challengerPolicy === 'none' || !plan.challengerPolicy) && !result.success) score -= 2;
  return Math.max(1, Math.min(5, score));
}

export function generateLessons(plan: Plan, result: Result & { duration?: number }, verification: Verification): string[] {
...
export async function recordOutcome(plan: Plan, result: Result & { duration?: number }, verification: Verification, cwd: string): Promise<OutcomeRecord | null> {
  try {
    ensureDir(cwd);
...
function promptOverlap(a: string | undefined, b: string | undefined): number {
...
export async function getRelevantOutcomes(prompt: string, files: string[] = [], cwd: string, options: Record<string, unknown> = {}): Promise<RelevantOutcome[]> {
  try {
...
export async function checkFileSurvival(cwd: string): Promise<SurvivalResult[]> {
  try {
    const dir = join(cwd, '.dualbrain', 'outcomes');
...
export async function getOutcomeStats(cwd: string, days: number = 7): Promise<OutcomeStats> {
  try {
    ...
    const modelBreakdown: Record<string, { count: number; successCount?: number; successRate: number }> = {};
    for (const o of outcomes) {
      const model = o.primaryModel;
      if (!model) continue;
      if (!modelBreakdown[model]) modelBreakdown[model] = { count: 0, successCount: 0, successRate: 0 };
    ...
    return {
      ...
      challengerHelpRate,
      topLessons,
      modelBreakdown: modelBreakdown as Record<string, { count: number; successRate: number }>,
    };
  } catch {
    return {
      totalTasks: 0,
