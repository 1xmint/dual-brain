/**
 * doctor.ts — Diagnostic and recovery stage in the dual-brain pipeline.
...
interface DecisionAreaStatus {
  area: string;
  status: 'missing' | 'invalid' | 'expired' | 'active' | 'inactive';
...
interface FoundationIssue {
  type: string;
  file: string[];
  activeFoundation: string;
  invalidatedFoundation: string;
}
...
interface TokenWasteFinding {
  severity: 'warn';
  type: 'token-waste';
  message: string;
  file: null;
  nonDispatchCalls: number;
  totalCalls: number;
}

interface DoctorResults {
  claims: { issues: ClaimIssue[] };
  decisions: { areas: DecisionAreaStatus[] };
  foundations: FoundationsResult;
  roleBoundaries: RoleBoundaryFinding[];
  evidence: EvidenceFinding[];
  tokenWaste: TokenWasteFinding[];
  summary: { issueCount: number; blockingCount: number; verdict: 'pass' | 'issues' | 'fail' };
}
...
interface HealthCheckResult {
  ...
  staticChecks: DoctorResults | null;
  checkedAt: string;
}
...
interface PipelineRun {
  ...
  plan?: { useChallenger?: boolean; [key: string]: unknown } | null;
}
...
interface VerifierEntry {
  ttl: number;
  fn: (cwd?: string) => { status: string; evidence: string; probe: string };
}
...
interface RoutingSuggestion {
  suggestion: 'keep' | 'simplify' | 'escalate';
  reason: string;
  confidence: number;
...
interface ReconcileResult {
  ...
  demotions: ReconcileDemotion[];
  sentinels: ReconcileSentinel[];
}
...
// ─── Helpers ─────────────────────────────────────────────────────────────────
async function mjsFilesIn(dir: string): Promise<string[]> {
  try {
...
function readAuditLines(cwd: string): string[] {
...
export async function scanClaims(cwd: string): Promise<{ issues: ClaimIssue[] }> {
  const allFiles = [
  ...
  for (const filePath of allFiles) {
    let text: string; try { text = await readFile(filePath, 'utf8'); } catch { continue; }
...
const SENSITIVE_AREAS: { pattern: RegExp; area: string }[] = [
...
export async function checkDecisions(cwd: string): Promise<{ areas: DecisionAreaStatus[] }> {
  const decisionsDir = join(cwd, '.dualbrain', 'decisions');
  const today = new Date().toISOString().slice(0, 10);
  const seen = new Set<string>();
  const areas: DecisionAreaStatus[] = [];
  for (const { area } of SENSITIVE_AREAS) {
    ...
    let artifact: { expires_at?: string; status?: string; decided_at?: string };
...
export async function checkFoundations(cwd: string): Promise<FoundationsResult> {
  const manifestPath = join(cwd, '.dualbrain', 'foundations.json');
  if (!existsSync(manifestPath)) return { foundations: [], issues: [], missing: true };
  let data: { foundations?: Array<{ id: string; claim: string; status: string; dependents?: string[] }> };
  try { data = JSON.parse(readFileSync(manifestPath, 'utf8')); }
  catch { return { foundations: [], issues: [{ type: 'parse-error', file: [], activeFoundation: '', invalidatedFoundation: '', } as unknown as FoundationIssue], missing: false }; }
  const all = data.foundations || [];
  const issues: FoundationIssue[] = [];
  const foundations: FoundationEntry[] = all.map(f => {
    const entry: FoundationEntry = { id: f.id, claim: f.claim, status: f.status, dependents: f.dependents || [] };
...
export async function checkRoleBoundaries(cwd: string): Promise<RoleBoundaryFinding[]> {
  const lines = readAuditLines(cwd);
  const findings: RoleBoundaryFinding[] = [];
  for (const line of lines) {
    let entry: { ts?: string; tool?: string; event?: string; reason?: string; allowed?: boolean };
...
export async function checkEvidence(cwd: string): Promise<EvidenceFinding[]> {
  const outcomesDir = join(cwd, '.dualbrain', 'outcomes');
  if (!existsSync(outcomesDir)) return [];
  let files: string[]; try { files = await readdir(outcomesDir); } catch { return []; }
  const findings: EvidenceFinding[] = [];
  for (const fname of files.filter(f => f.endsWith('.json')).slice(-20)) {
    let outcome: { filesChanged?: string[]; success?: boolean; testsRun?: boolean; testOutput?: string; testSummary?: string };
    try { outcome = JSON.parse(await readFile(join(outcomesDir, fname), 'utf8')); } catch { continue; }
...
export async function checkTokenWaste(cwd: string): Promise<TokenWasteFinding[]> {
  const lines = readAuditLines(cwd);
  let total = 0, nonDispatch = 0;
  for (const line of lines) {
    let entry: { event?: string; tool?: string; reason?: string };
    try { entry = JSON.parse(line); } catch { continue; }
...
// ─── Orchestrator ─────────────────────────────────────────────────────────────
export async function runDoctor(cwd: string = process.cwd()): Promise<DoctorResults> {
  ...
  const verdict: 'pass' | 'issues' | 'fail' = totalBlocking > 0 ? 'fail' : (legacyIssues + warnCount > 0 ? 'issues' : 'pass');
...
// ─── Formatter ────────────────────────────────────────────────────────────────
function section(out: string[], title: string, items: string[] | null, emptyMsg: string): void {
  out.push(`${title}:`);
...
export function formatDoctorReport(results: DoctorResults): string {
  const { claims, decisions, foundations, roleBoundaries, evidence, tokenWaste, summary } = results;
  const out: string[] = ['dual-brain doctor', ''];
...
interface HealthManifestItem {
  ...
  status?: string;
  lastChecked?: string;
  verification?: { type?: string; command?: string; expect?: string };
}

function runVerification(item: HealthManifestItem): { status: string; detail: string } {
  ...
  try {
    ...
    return { status: 'fail', detail: (e.stderr || e.stdout || e.message || '').toString().trim().slice(0, 200) };
  }
}

function domainStats(items: HealthManifestItem[]): Record<string, DomainStat> {
  const domains: Record<string, { score: number; total: number; passed: number; wt: number; wp: number }> = {};
  ...
  const result: Record<string, DomainStat> = {};
  for (const [d, v] of Object.entries(domains)) {
    result[d] = {
      score: v.wt > 0 ? Math.round((v.wp / v.wt) * 100) : 0,
      total: v.total,
      passed: v.passed,
    };
  }
...
export async function runHealthCheck(cwd: string = process.cwd(), mode: string = 'quick'): Promise<HealthCheckResult> {
  const mpath = join(cwd, '.dualbrain', 'health-manifest.json');
  const manifest = existsSync(mpath) ? (() => { try { return JSON.parse(readFileSync(mpath, 'utf8')); } catch { return null; } })() as { items?: HealthManifestItem[]; [key: string]: unknown } | null : null;
  const items: HealthManifestItem[] = manifest ? (manifest.items || []) : [];
...
function bar(passed: number, total: number, w: number = 10): string {
...
export function formatHealthReport(results: HealthCheckResult): string {
  ...
  const out: string[] = [`🩺 Health Report — ${score}/100`, ''];
...
export async function doctorDiagnose(run: PipelineRun): Promise<DiagnoseResult> {
...
export async function doctorRecover(run: PipelineRun, failure: FailureContext = {}): Promise<RecoverResult> {
...
const VERIFIERS: Record<string, VerifierEntry> = {
  ...
  'living-docs-init': { ttl: TTL_RUNTIME, fn: (cwd?: string) => {
 ...
 */
export function verify(claim: string, cwd?: string): VerificationResult {
  const checkedAt = new Date().toISOString();
  ...
  try {
    ...
    return { claim, status: result.status as VerificationResult['status'], evidence: result.evidence, checkedAt, expiresAt, probe: result.probe };
  } catch (err: unknown) {
    const expiresAt = new Date(Date.now() + TTL_RUNTIME).toISOString();
    const message = err instanceof Error ? err.message : String(err);
    return { claim, status: 'unknown', evidence: `Verifier threw: ${message}`, checkedAt, expiresAt, probe: 'error' };
  }
}
 ...
 */
export function verifyAll(cwd: string = process.cwd()): VerificationResult[] {
 ...
 */
export function getVerificationCache(cwd: string = process.cwd()): VerificationResult[] {
  ...
  const latest: Record<string, VerificationResult> = {};

  for (const line of lines) {
    let entry: VerificationResult;
    try { entry = JSON.parse(line) as VerificationResult; } catch { continue; }
 ...
 */
export function getStaleAssumptions(cwd: string = process.cwd()): StaleAssumption[] {
  const cached = getVerificationCache(cwd);
  const cachedMap: Record<string, VerificationResult> = Object.fromEntries(cached.map(r => [r.claim, r]));
 ...
 */
export function formatVerifications(results: VerificationResult[]): string {
...
function readLearnings(cwd: string): LearningRecord[] {
  const p = learningsPath(cwd);
  if (!existsSync(p)) return [];
  try {
    return readFileSync(p, 'utf8').trim().split('\n').filter(Boolean).flatMap(line => {
      try { return [JSON.parse(line) as LearningRecord]; } catch { return []; }
    });
  } catch { return []; }
}

function deriveModelFit(taskResult: { success: boolean; model: string; tier: string; taskType: string; duration: number; filesChanged: number }): string {
...
function deriveRoutingAccuracy(taskResult: { success: boolean; modelFit: string; tier: string; duration: number; model: string }): string {
...
export function recordLearning(taskResult: TaskResult, cwd: string = process.cwd()): LearningRecord | null {
  try {
    const {
      taskType = 'unknown',
      prompt = '',
...
export function getModelSuccessRates(cwd: string = process.cwd(), days: number = 7): Record<string, ModelSuccessRate> {
  const cutoff = new Date(Date.now() - days * 86400000).toISOString();
  const learnings = readLearnings(cwd).filter(l => l.timestamp >= cutoff);

  const stats: Record<string, { total: number; success: number; totalDuration: number; tierCounts: Record<string, number> }> = {};
  ...
  const result: Record<string, ModelSuccessRate> = {};
...
export function getRoutingInsights(cwd: string = process.cwd()): RoutingInsight[] {
  ...
  const byModelTask: Record<string, { success: number; total: number; overkill: number; underpowered: number }> = {};
  for (const l of learnings) {
...
export function suggestRoutingAdjustment(taskType: string, currentModel: string, cwd: string = process.cwd()): RoutingSuggestion {
...
export function formatLearnings(insights: RoutingInsight[] | undefined, cwd: string = process.cwd()): string {
  const learnings = readLearnings(cwd);
...
export function getLearningStats(cwd: string = process.cwd()): LearningStats {
  const learnings = readLearnings(cwd);
...
function safeExecSyncDiscover(cmd: string): string | null {
  try {
    return execSync(cmd, { timeout: 2000, encoding: 'utf8', stdio: ['pipe','pipe','pipe'] }).trim();
  } catch { return null; }
}

function discoverCLITools(): DiscoveredCapability[] {
  const found: DiscoveredCapability[] = [];
 ...
 */
export function discoverMCPTools(cwd: string = process.cwd()): MCPServer[] {
  const locations = [
  ...
  for (const loc of locations) {
    ...
    let cfg: { mcpServers?: Record<string, { command?: string; args?: string[] }>; mcp?: { servers?: Record<string, { command?: string; args?: string[] }> } };
...
function discoverMCPCapabilities(cwd: string): DiscoveredCapability[] {
...
function discoverEnvServices(): DiscoveredCapability[] {
  const found: DiscoveredCapability[] = [];
  const seen = new Set<string>();
...
function discoverProjectTools(cwd: string): DiscoveredCapability[] {
...
function discoverReplitFeatures(cwd: string): DiscoveredCapability[] {
...
function loadLastDiscovery(cwd: string): DiscoveryResult | null {
  const logPath = join(cwd, '.dualbrain', 'discoveries.jsonl');
  if (!existsSync(logPath)) return null;
  try {
    const lines = readFileSync(logPath, 'utf8').trim().split('\n').filter(Boolean);
    if (lines.length === 0) return null;
    return JSON.parse(lines[lines.length - 1]) as DiscoveryResult;
  } catch { return null; }
}

function appendDiscoveryLog(cwd: string, entry: DiscoveryResult): void {
  const dir = join(cwd, '.dualbrain');
  try {
 ...
 */
export function discover(cwd: string = process.cwd()): DiscoveryResult {
  const discoveredAt = new Date().toISOString();
  const allFound: DiscoveredCapability[] = [];
 ...
 */
export function getDiscoveryLog(cwd: string = process.cwd(), limit: number = 20): DiscoveryResult[] {
  const logPath = join(cwd, '.dualbrain', 'discoveries.jsonl');
  if (!existsSync(logPath)) return [];
  try {
    const lines = readFileSync(logPath, 'utf8').trim().split('\n').filter(Boolean);
    return lines.slice(-limit).map(l => { try { return JSON.parse(l) as DiscoveryResult; } catch { return null; } }).filter((v): v is DiscoveryResult => v !== null);
  } catch { return []; }
}
 ...
 */
export function getNewSinceLastScan(cwd: string = process.cwd()): DiscoveryResult {
  const last = loadLastDiscovery(cwd);
  const lastNames = new Set(last ? (last.newCapabilities || []).map(c => `${c.type}:${c.name}`) : []);
...
function ensureDoctorDir(cwd: string): void {
 ...
 */
export function recordEvent(event: DoctorEvent, cwd: string = process.cwd()): DoctorEvent | null {
  try {
    ensureDoctorDir(cwd);
    const entry: DoctorEvent = {
 ...
 */
export function getRecentEvents(cwd: string = process.cwd(), days: number = 7): DoctorEvent[] {
  const p = eventsPath(cwd);
  if (!existsSync(p)) return [];
  const cutoff = new Date(Date.now() - days * 86400000).toISOString();
  try {
    return readFileSync(p, 'utf8').trim().split('\n').filter(Boolean)
      .flatMap(line => { try { return [JSON.parse(line) as DoctorEvent]; } catch { return []; } })
 ...
 */
export function getEventsForCheck(checkId: string, cwd: string = process.cwd()): DoctorEvent[] {
  const p = eventsPath(cwd);
  if (!existsSync(p)) return [];
  try {
    return readFileSync(p, 'utf8').trim().split('\n').filter(Boolean)
...
const STATIC_CHECK_SEEDS: CheckSeed[] = [
...
function checkSpecPath(checkId: string, cwd: string): string {
  return join(checksDir(cwd), `${checkId}.json`);
}

function defaultSpec(seed: CheckSeed): CheckSpec {
  return {
    id: seed.id,
    kind: seed.kind,
 ...
 */
export function getCheckRegistry(cwd: string = process.cwd()): CheckSpec[] {
  try {
    ensureDoctorDir(cwd);
    ...
    return entries.flatMap(fname => {
      try { return [JSON.parse(readFileSync(join(checksDir(cwd), fname), 'utf8')) as CheckSpec]; }
      catch { return []; }
    });
  } catch { return []; }
}
 ...
 */
export function registerCheck(spec: Partial<CheckSpec> & { id: string }, cwd: string = process.cwd()): CheckSpec {
  if (!spec || !spec.id) throw new Error('registerCheck: spec.id is required');
  try {
    ensureDoctorDir(cwd);
    const p = checkSpecPath(spec.id, cwd);
    const existing: CheckSpec | null = existsSync(p)
      ? (() => { try { return JSON.parse(readFileSync(p, 'utf8')) as CheckSpec; } catch { return null; } })()
      : null;
    const merged: CheckSpec = existing ? { ...existing, ...spec } as CheckSpec : { ...defaultSpec(spec as CheckSeed), ...spec } as CheckSpec;
    writeFileSync(p, JSON.stringify(merged, null, 2) + '\n', 'utf8');
    return merged;
  } catch (e: unknown) {
    const message = e instanceof Error ? e.message : String(e);
    throw new Error(`registerCheck failed: ${message}`);
  }
}
 ...
 */
export function updateCheckStats(checkId: string, outcome: 'pass' | 'fail' | 'warn' | 'false_positive', cwd: string = process.cwd()): void {
  try {
    ensureDoctorDir(cwd);
    const p = checkSpecPath(checkId, cwd);
    if (!existsSync(p)) return; // not registered — skip silently
    let spec: CheckSpec; try { spec = JSON.parse(readFileSync(p, 'utf8')) as CheckSpec; } catch { return; }
 ...
 */
export function getCheckHealth(cwd: string = process.cwd()): CheckHealthEntry[] {
  const registry = getCheckRegistry(cwd);
  return registry.map(spec => {
    const sig = spec.signal || {} as CheckSignal;
 ...
 */
export function reconcile(cwd: string = process.cwd()): ReconcileResult {
  const proposals: ReconcileProposal[] = [];
  const demotions: ReconcileDemotion[] = [];
  const sentinels: ReconcileSentinel[] = [];

  try {
    const recentEvents = getRecentEvents(cwd, 7);
    const registry = getCheckRegistry(cwd);
    const checkMap: Record<string, CheckSpec> = Object.fromEntries(registry.map(c => [c.id, c]));
...
function _inferPrimitive(evidence: string): string | null {
...
export async function compareHealth(cwd: string = process.cwd()): Promise<HealthComparison> {
  const bpath = join(cwd, '.dualbrain', 'health-baseline.json');
  let baseline: HealthCheckResult | null = null;
  if (existsSync(bpath)) { try { baseline = JSON.parse(readFileSync(bpath, 'utf8')) as HealthCheckResult; } catch { /* ignore */ } }

  const current = await runHealthCheck(cwd, 'quick');
  const regressions: string[] = [], improvements: string[] = [];

  if (baseline && baseline.findings) {
    const prev: Record<string, string> = Object.fromEntries(baseline.findings.map(f => [f.id, f.status]));
