...
function safeJson(filePath: string): Record<string, unknown> | null {
...
function findReplitToolsDir(cwd: string): string | null {
  const candidates = [
...
export function inspectReplitConfig(cwd = process.cwd()) {
  ...
  const nix: Record<string, string> = {};
...
export function inspectReplitTools(cwd = process.cwd()) {
  ...
  if (existsSync(archiveBase)) {
    ...
    function countJsonl(dir: string): void {
...
function looksLikeSystemVar(name: string): boolean {
...
export function getSessionArchive(cwd = process.cwd()) {
  ...
  const sessions: Array<{ id: string; path: string; size: number; lastModified: string }> = [];
 ...
 */
export function planReplitConfig(desired: Record<string, unknown> = {}, cwd = process.cwd()) {
  ...
  const wantExpertMode = (desired as Record<string, unknown>).expertMode ?? false;
  ...
  const desiredHidden = (desired.hidden ?? DUAL_BRAIN_HIDDEN) as string[];
  const missingHidden = desiredHidden.filter((h: string) => !currentHidden.has(h));
  ...
  if ((desired as Record<string, unknown>).onBoot !== undefined) {
    if (current.onBoot !== (desired as Record<string, unknown>).onBoot) {
      changes.push({
        key: 'onBoot',
        action: 'set',
        value: (desired as Record<string, unknown>).onBoot,
  ...
  if (current.hasRun && (desired as Record<string, unknown>).removeRun !== false) {
  ...
  const actionSummary = changes.map(c => {
    ...
    if (c.action === 'add') return `add ${(c.adds as string[])?.join(', ')} to ${c.key}`;
 ...
 */
function applyReplitChanges(changes: Array<Record<string, unknown>>, cwd: string, { skipMedium = false } = {}): string[] {
  ...
  for (const change of changes) {
    ...
    if (change.key === 'expertMode' && change.action === 'remove') {
      ...
      const valueStr = formatTomlArray(change.value as string[]);
...
function formatTomlArray(items: string[]): string {
  return '[' + items.map((i: string) => `"${i}"`).join(', ') + ']';
}

function replaceOrInsertTopLevel(lines: string[], key: string, valueStr: string): string[] {
  ...
  if (!found) {
    ...
    const firstSection = result.findIndex((l: string) => /^\s*\[/.test(l));
    if (firstSection > 0) {
...
function cleanEmptySection(lines: string[], sectionName: string): string[] {
...
export function initReplitIntegration({ dryRun = false, cwd = process.cwd() } = {}) {
  ...
  if (!dryRun && plan.changes.length) {
    try {
      applied = applyReplitChanges(plan.changes as Array<Record<string, unknown>>, resolvedCwd);
    } catch (err) {
      applied = [];
    }
  }
 ...
 */
export function runReplitCli(args: string[], options: { timeout?: number } = {}) {
 ...
 * @returns {string}
 */
export function formatReplitReport(report: Record<string, unknown>): string {
  const environment = report.environment as Record<string, unknown>;
  const replitTools = report.replitTools as Record<string, unknown>;
  const config = report.config as Record<string, unknown>;
  const plan = report.plan as Record<string, unknown>;
  const applied = report.applied as string[];
  const dryRun = report.dryRun as boolean;
  const lines = [];
  ...
  if (replitTools.installed) {
    ...
    const sessionArchive = replitTools.sessionArchive as Record<string, unknown>;
    const codexPlugins = replitTools.codexPlugins as Record<string, unknown>;
    const shellSnapshots = replitTools.shellSnapshots as Record<string, unknown>;
    const mcpAuthCache = replitTools.mcpAuthCache as Record<string, unknown>;
    if (sessionArchive.exists) {
      const tsLabel = sessionArchive.latestTimestamp
        ? ` latest: ${new Date(sessionArchive.latestTimestamp as number).toLocaleDateString()}`
        : '';
      lines.push(`  sessions: ${sessionArchive.sessionCount}${tsLabel}`);
    }
    if (Number(codexPlugins.count) > 0) lines.push(`  codex plugins: ${codexPlugins.count}`);
    if (Number(shellSnapshots.count) > 0) lines.push(`  shell snapshots: ${shellSnapshots.count}`);
    if (Number(mcpAuthCache.entries) > 0) lines.push(`  mcp cached: ${mcpAuthCache.entries} servers`);
    if (replitTools.toolsConfig) {
      lines.push(`  session window: ${(replitTools.toolsConfig as Record<string, unknown>).recentWindowHours}h`);
    }
  } else {
  ...
  if (config.raw === null) {
    ...
    if ((config.hidden as string[])?.length) lines.push(`  hidden: ${(config.hidden as string[]).join(', ')}`);
    if (config.hasOnBoot) lines.push(`  onBoot: ${((config.onBoot as string) || '').slice(0, 60)}…`);
    if ((config.modules as string[])?.length) lines.push(`  modules: ${(config.modules as string[]).join(', ')}`);
  }

  // Plan
  if ((plan.changes as unknown[]).length === 0) {
    lines.push('Config: already optimal');
  } else {
    lines.push(`Plan (${dryRun ? 'dry-run' : plan.riskLevel} risk):`);
    for (const c of plan.changes as Array<Record<string, unknown>>) {
      const prefix = `  [${c.risk}]`;
      if (c.action === 'remove') lines.push(`${prefix} remove ${c.key} — ${c.reason || ''}`);
      else if (c.action === 'add') lines.push(`${prefix} add to ${c.key}: ${(c.adds as string[] | undefined)?.join(', ')}`);
      else if (c.action === 'set') lines.push(`${prefix} set ${c.key}`);
    }
    if ((plan.preserves as string[])?.length) lines.push(`  preserves: ${(plan.preserves as string[]).join('; ')}`);
  }

  // Applied
  if (!dryRun && applied?.length > 0) {
    lines.push(`Applied: ${applied.join(', ')}`);
  } else if (!dryRun && (plan.changes as unknown[]).length > 0) {
...
let _pluginInventoryCache: { plugins: Array<{ id: string; name: string; description: string; capabilities: string[]; skillNames: string[]; path: string }>; count: number } | null = null;
 ...
 */
function _parseFrontmatter(content: string): { name?: string; description?: string } {
  ...
  for (const pluginId of safeReaddir(pluginsDir)) {
    ...
    for (const skillDir of skillDirs) {
      ...
      if (fm.description) {
        ...
        if (triggerMatch) {
          const triggers = triggerMatch[1].split(/[,;]+/).map((s: string) => s.trim()).filter((s: string) => s.length > 1 && s.length < 30);
 ...
 */
export function matchPluginsForTask(taskDescription: string, plugins?: Array<{ id: string; name: string; description: string; capabilities: string[]; skillNames: string[]; path: string }>, cwd: string = process.cwd()) {
 ...
 */
export async function searchSessionArchive(query: string, options: { limit?: number; days?: number } = {}, cwd: string = process.cwd()) {
  ...
  const matches: Array<{ sessionId: string; date: string; matchingMessage: string; relevance: number }> = [];
