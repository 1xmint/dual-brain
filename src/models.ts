/**
 * models.ts — Static model intelligence registry for the Dual-Brain Orchestrator.
 *
...
export interface ModelEntry {
  ...
  name: string;
  tier: 'fast' | 'workhorse' | 'reasoning' | 'frontier';
...
const COST_TIER_ORDER = ['low', 'medium', 'high', 'premium'] as const;
const SPEED_ORDER = ['fast', 'medium', 'slow'] as const;
const TIER_ORDER = ['fast', 'workhorse', 'reasoning', 'frontier'] as const;

const MODEL_QUIRKS: Record<string, string[]> = {
  'claude-opus-4-6': [
...
export interface ModelWithId extends ModelEntry {
  id: string;
}
...
export function getModelsForTask(taskType: string, provider: string | null = null): ModelWithId[] {
...
export function getBestModel(taskType: string, constraints: ModelConstraints = {}): ModelWithId | null {
  ...
  if (maxCost) {
    const maxIdx = COST_TIER_ORDER.indexOf(maxCost as typeof COST_TIER_ORDER[number]);
    if (maxIdx !== -1) {
...
export interface ModelComparison {
  contextAdvantage: 'a' | 'b' | 'tie';
  speedAdvantage: 'a' | 'b' | 'tie';
  costAdvantage: 'a' | 'b' | 'tie';
  reasoningAdvantage: 'a' | 'b' | 'tie';
...
export function compareModels(modelA: string, modelB: string): ModelComparison {
  ...
  const ctxAdv: 'a' | 'b' | 'tie' = a.contextWindow > b.contextWindow ? 'a'
    : b.contextWindow > a.contextWindow ? 'b'
    : 'tie';

  const spdA = SPEED_ORDER.indexOf(a.speed);
  const spdB = SPEED_ORDER.indexOf(b.speed);
  const spdAdv: 'a' | 'b' | 'tie' = spdA < spdB ? 'a' : spdB < spdA ? 'b' : 'tie';

  const costA = COST_TIER_ORDER.indexOf(a.costTier);
  const costB = COST_TIER_ORDER.indexOf(b.costTier);
  const costAdv: 'a' | 'b' | 'tie' = costA < costB ? 'a' : costB < costA ? 'b' : 'tie';

  const rsnAdv: 'a' | 'b' | 'tie' = (a.reasoning && !b.reasoning) ? 'a'
...
export function getRegistryAge(): number {
  const updated = new Date(REGISTRY_UPDATED);
  const now = new Date();
  return Math.floor((now.getTime() - updated.getTime()) / (1000 * 60 * 60 * 24));
}

const COST_SYMBOL: Record<string, string> = { low: '$', medium: '$$', high: '$$$', premium: '$$$$' };
...
export function formatModelTable(provider: string | null = null): string {
  const groups: Record<string, ModelWithId[]> = {};
  ...
  const lines: string[] = [];
  const providerLabels: Record<string, string> = { anthropic: 'Claude Models', openai: 'OpenAI Models' };
...
export function getModelQuirks(modelId: string): string[] {
...
export function suggestModel(intent: string, risk: string, complexity: string, availableProviders: string[] = ['anthropic', 'openai']): ModelSuggestion {
  ...
  const candidates = getModelsForTask(taskType)
    ...
    .filter(m => !maxCost || COST_TIER_ORDER.indexOf(m.costTier) <= COST_TIER_ORDER.indexOf(maxCost as typeof COST_TIER_ORDER[number]));
