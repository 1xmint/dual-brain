...
function getRecentCommits(cwd: string, n = 5): string[] {
  const raw = safeExec(`git log -${n} --pretty=format:%s`, cwd);
  if (!raw) return [];
  return raw.split('\n').filter(Boolean);
}

function getAheadCount(cwd: string): number {
...
function readRecentFailures(cwd: string, limit = 10): ParsedFailure[] {
  ...
  try {
    ...
    return lines
      ...
      .reverse()
      .map(line => {
        try { return JSON.parse(line) as FailureRecord; } catch { return null; }
      })
      .filter((r): r is FailureRecord => r !== null && !r.resolved)
      .map(r => ({
        prompt: r.prompt ?? '',
        error: r.error ?? '',
...
function readRecentOutcomes(cwd: string, limit = 10): ParsedOutcome[] {
 ...
 */
export function deriveProjectState(cwd = process.cwd()): ProjectBrief {
...
export interface TaskBrief {
  task: string;
  priorAttempts: Array<{ approach: string; failed: boolean; reason: string }>;
 ...
 */
export function deriveTaskContext(task = '', recentEvents: Array<PriorAttempt | null> = []): TaskBrief {
  const priorAttempts: Array<{ approach: string; failed: boolean; reason: string }> = [];
  const filesOutOfScope: string[] = [];
  const filesInScopeSet = new Set<string>();
  ...
  const escalationLevel: 'normal' | 'elevated' | 'critical' =
 ...
 */
export function detectContradictions(projectBrief: ProjectBrief, taskBrief: TaskBrief | null, plan: Plan = {}): Contradiction[] {
 ...
 */
export function formatBrief(projectBrief: ProjectBrief, taskBrief: TaskBrief | null, sessionContext: SessionContext | null = null): string {
  const lines: string[] = [];
