...
interface ClaudeCodeInfo {
  isInsideClaude: boolean;
  hooksDir: string | null;
  mcpConfigured: boolean;
  settingsPath: string | null;
}

interface DualBrainInfo {
  version: string;
  livingDocsInit: boolean;
  sessionCount: number;
  hasLedger: boolean;
...
export interface EnvironmentReport {
  ...
  replitTools: ReplitToolsInfo;
  replit: ReplitInfo;
  claudeCode: ClaudeCodeInfo;
  dualBrain: DualBrainInfo;
}
...
let _cache: EnvironmentReport | null = null;
let _cacheTime = 0;

// ─── Helpers ─────────────────────────────────────────────────────────────────

function safeExec(cmd: string, timeoutMs: number = 2000): string | null {
  try {
    return execSync(cmd, {
      encoding: 'utf8',
...
function probeToolAvailability(name: string): ToolInfo {
...
function detectContainerType(): string {
...
function scanSecrets(): Record<string, boolean> {
  const keys = [
    'NPM_TOKEN',
    'DATABASE_URL',
    'GITHUB_TOKEN',
...
function scanReplitTools(): ReplitToolsInfo {
  const home = homedir();
  const candidates = [
  ...
  let sessionArchivePath: string | null = null;
 ...
 *   }
 */
function scanReplit(cwd: string): ReplitInfo {
  const env = process.env;
...
function scanClaudeCode(cwd: string): ClaudeCodeInfo {
...
function scanDualBrain(cwd: string): DualBrainInfo {
  let version = '0.0.0';
  try {
...
export function scanEnvironment(cwd: string | undefined, options: ScanOptions = {}): EnvironmentReport {
...
export function getCapabilitySummary(report: EnvironmentReport): string[] {
  const caps: string[] = [];
 ...
 */
export function detectAmbiguity(prompt: unknown, context: { files?: string[] } = {}): AmbiguityResult {
 ...
 */
export function isAmbiguous(prompt: unknown): AmbiguousResult {
