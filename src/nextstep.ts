...
function packageVersionChanged(cwd: string, files: string[]): boolean {
...
function changelogExists(cwd: string): boolean {
  return ['CHANGELOG.md', 'CHANGELOG', 'changelog.md'].some(f => existsSync(join(cwd, f)));
}

function step(priority: number, type: string, message: string, command: string | null, reason: string): { priority: number; type: string; message: string; command: string | null; reason: string } {
  return { priority, type, message, command, reason };
}

function dedup(steps: Array<{ priority: number; type: string; message: string; command: string | null; reason: string }>): Array<{ priority: number; type: string; message: string; command: string | null; reason: string }> {
  const seen = new Set();
  return steps.filter(s => { if (seen.has(s.type)) return false; seen.add(s.type); return true; });
}

export async function suggestNextSteps(completedTask: Record<string, unknown> = {}, outcome: Record<string, unknown> = {}, cwd: string = process.cwd()) {
  try {
    const { prompt = '', files = [], trigger } = completedTask as { prompt?: string; files?: string[]; trigger?: string };
    const { success = false, filesChanged = [], error = '' } = outcome as { success?: boolean; filesChanged?: string[]; error?: string };
    const steps = [];
    ...
    const fstr = (count: number) => `${count} file${count !== 1 ? 's' : ''}`;
    ...
    if (trigger === 'auto-commit') {
      ...
      if (!hasTests)
        steps.push(step(5, 'test', `Run tests to verify the ${fstr(n)} changed`, 'npm test', `${fstr(n)} changed without test verification`));
      if (hasAuth)
...
export function formatNextSteps(steps: Array<{ message: string }> | undefined, limit = 3): string {
  if (!steps?.length) return '';
  return `📋 Next steps\n${steps.slice(0, limit).map((s, i) => `  ${i + 1}. ${s.message}`).join('\n')}`;
}

export function getTopSuggestion(steps: Array<{ message: string }> | undefined): string {
