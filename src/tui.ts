 ...
 * @returns {string}
 */
export function box(title: string, lines: string[] = [], opts: { width?: number } = {}): string {
 ...
 * @returns {string}
 */
export function bar(percent: number, width = 20, opts: { label?: string } = {}): string {
 ...
 * @returns {string}
 */
export function menu(options: Array<{ key: string; label: string; section?: string }>, opts: Record<string, unknown> = {}): string {
  const rows: string[] = [];
  let lastSection: string | symbol = Symbol('none');

  for (const opt of options) {
    const section = opt.section ?? '';
    if (section !== (lastSection as string)) {
      if (section) {
...
export function panel(title: string, content: string | string[], opts: { width?: number; titleColor?: string; borderColor?: string; reset?: string } = {}): string {
...
export function divider(width = 70): string {
  const borderColor = '\x1b[2m';
  const reset = '\x1b[0m';
  return `${borderColor}${ROUNDED.ml}${ROUNDED.h.repeat(width - 2)}${ROUNDED.mr}${reset}`;
}

export function statusChip(label: string, healthy: boolean, opts: Record<string, unknown> = {}): string {
  const green = '\x1b[32m';
  const red = '\x1b[31m';
  const dim = '\x1b[2m';
  const reset = '\x1b[0m';
...
export function prompt(text = '> task or /help'): string {
  const cyan = '\x1b[36m';
  const dim = '\x1b[2m';
  const reset = '\x1b[0m';
...
export function signalLine(type: string, text: string, meta = ''): string {
  const green = '\x1b[32m';
