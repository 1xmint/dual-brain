 ...
 */
export function add(idea: string, { origin = 'observation', initialHeat = 1 }: { origin?: string; initialHeat?: number } = {}): SimmerItem {
  const items = _load();
 ...
 */
export function heat(id: string, amount = 1, signal = ''): SimmerItem | null {
  const items = _load();
 ...
 * @returns {SimmerItem[]}
 */
export function harvest(): SimmerItem[] {
  const items = _load();
 ...
 */
export function resolve(id: string, became: string): void {
  const items = _load();
  const item = items.find(i => i.id === id);
  if (!item) return;
 ...
 * @returns {SimmerItem[]}
 */
export function active(): SimmerItem[] {
  const items = _load();
  _applyDecay(items);
  return items
 ...
 * @returns {SimmerItem|null}
 */
export function find(idea: string): SimmerItem | null {
  const items = _load();
...
function _save(items: SimmerItem[]): void {
  // Cap total items
...
function _applyDecay(items: SimmerItem[]): void {
  const now = Date.now();
...
function _findSimilar(items: SimmerItem[], idea: string): SimmerItem | null {
  ...
  for (const item of items) {
    ...
    const itemNorm = (item.idea as string).toLowerCase().replace(/[^a-z0-9\s]/g, '');
