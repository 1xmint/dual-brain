...
interface SessionPatterns {
  frequencies: Array<{ type: string; count: number }>;
  avgSeverity: number;
  precedingTools: Record<string, string[]>;
  timeTrends: Array<{ type: string; position: number }>;
}

export function loadSessionPatterns(): SessionPatterns {
  const empty: SessionPatterns = { frequencies: [], avgSeverity: 0, precedingTools: {}, timeTrends: [] };
  ...
  const precedingTools: Record<string, string[]> = {};
  for (const n of noticings) {
...
function loadWeights(): Record<string, PatternWeightEntry> {
...
interface PredictionContext {
  ...
  priorFailures?: Array<{ objective?: string }>;
  activeWaves?: Array<{ output?: string }>;
  lastReadAge?: number;
  priorSearchWave?: boolean;
}
...
export function predictFailureModes(agentSpec: AgentSpec, context: PredictionContext = {}): Prediction[] {
  ...
  const applyWeight = (mode: string, base: number): number => {
...
export function generatePreventions(predictions: Prediction[]): string {
...
export function scoreDispatchReadiness(agentSpec: AgentSpec, wavePlan: WavePlan = {}, predictions: Prediction[] = []): ReadinessScore {
...
export function evolvePatterns(newDebrief: Debrief, predictions: Prediction[]): void {
  const weights = loadWeights();
...
function debriefContainsPattern(debrief: Debrief | null | undefined, mode: string): boolean {
