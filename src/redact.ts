// redact.ts — Secret redaction utility for dual-brain orchestrator.
...
const REDACT_PATTERNS: RedactPattern[] = [
  // .env-style: KEY=VALUE (key contains KEY|SECRET|TOKEN|PASSWORD|CREDENTIAL|AUTH)
  {
    ...
    replacer: (_m: string, key: string, _kw: string, val: string) => isPlaceholder(val) ? _m : `${key}[REDACTED]`,
  },
  ...
  // Explicit named keys (case-insensitive) with = or : assignment
  {
    ...
    replacer: (_m: string, prefix: string, val: string, suffix: string) => isPlaceholder(val) ? _m : `${prefix}[REDACTED]${suffix}`,
  },
  ...
  // Bearer tokens: Bearer xxx / Authorization: Bearer xxx
  {
    ...
    replacer: (_m: string, prefix: string, val: string) => isPlaceholder(val) ? _m : `${prefix}[REDACTED]`,
  },

  // Authorization header value (non-Bearer forms)
  {
    pattern: /(Authorization\s*:\s*)([^\s\n][^\n]*)/gi,
    replacer: (_m: string, prefix: string, val: string) => {
  ...
  {
    pattern: /([\w+.-]+:\/\/[^:@\s]+:)([^@\s]+)(@)/g,
    replacer: (_m: string, prefix: string, pass: string, at: string) => isPlaceholder(pass) ? _m : `${prefix}[REDACTED]${at}`,
  },
 ...
 */
function redactFiles(filePaths: string[], cwd?: string): Set<string> {
