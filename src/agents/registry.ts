...
interface SkillDef {
  ...
  risk?: string;
  description: string;
  objective: (args: string) => string;
  acceptanceCriteria?: string[];
  scope?: string[];
}

let _agents: Map<string, AgentDef> | null = null;
let _skills: Map<string, SkillDef> | null = null;
...
export function matchAgent(intent: string, risk: string, taskType: string) {
  ...
  for (const [id, agent] of agents) {
    ...
    if (agent.taskTypes && agent.taskTypes.some((t: string) => taskType.includes(t))) score += 5;
...
export function matchSkill(command: string) {
  const skills = getSkills();
  const cmd = command.replace(/^\//, '').split(' ')[0].toLowerCase();
  return skills.get(cmd) || null;
}

export function skillToTaskBrief(command: string, args: string) {
...
function riskLevel(r: string): number {
  return ({ low: 1, medium: 2, high: 3, critical: 4 } as Record<string, number>)[r] || 0;
}

function parseAgentMd(content: string): AgentDef | null {
  ...
  return {
    ...
    intents: (meta.intents || '').split(',').map((s: string) => s.trim()),
    taskTypes: (meta.taskTypes || '').split(',').map((s: string) => s.trim()),
...
const BUILT_IN_SKILLS = [
  {
    ...
    objective: (args: string) => args ? `Review changes in ${args}` : 'Review all uncommitted changes',
  ...
  },
  {
    ...
    objective: (args: string) => `Debug: ${args || 'investigate recent error'}`,
  ...
  },
  {
    ...
    objective: (args: string) => `Generate comprehensive tests for ${args || 'changed files'}`,
  ...
  },
  {
    ...
    objective: (args: string) => `Create execution plan: ${args || 'current task'}`,
  ...
  },
  {
    ...
    objective: (args: string) => `Audit: ${args || 'security scan of current changes'}`,
  ...
  },
  {
    ...
    objective: (args: string) => `Fix: ${args}`,
  ...
  },
  {
    ...
    objective: (args: string) => `Explain: ${args || 'current file'}`,
  },
  {
    command: 'refactor',
    agent: 'refactor',
    ...
    objective: (args: string) => `Refactor: ${args || 'current file'}`,
  ...
  },
  {
    ...
    objective: (args: string) => `Document: ${args || 'changed files'}`,
  },
  {
    command: 'brainstorm',
    agent: 'architect',
    ...
    objective: (args: string) => `Brainstorm: ${args || 'current challenge'}`,
  },
  {
    command: 'search',
    agent: 'researcher',
    ...
    objective: (args: string) => `Search: ${args}`,
  },
  {
    command: 'status',
  ...
  },
  {
    ...
    objective: (args: string) => `Deploy: ${args || 'production'}`,
  ...
  },
  {
    ...
    objective: (args: string) => `Optimize performance: ${args || 'hot paths'}`,
  },
  {
    command: 'release',
    agent: 'docs',
    ...
    objective: (args: string) => `Prepare release: ${args || 'current version'}`,
