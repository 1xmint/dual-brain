#!/usr/bin/env node
// detect.ts — Task detection for dual-brain. Self-contained, no internal imports.
...
import type { Risk, Complexity, Tier, TaskDetection } from './types.js';
...
const DESIGN_IMPACT_PATTERNS: RegExp[] = [
...
const LEVEL_ORDER: Record<string, number> = { critical: 3, high: 2, medium: 1, low: 0 };

// ─── Helpers / Exported functions ─────────────────────────────────────────────

function higherRisk(a: string, b: string): Risk {
  return (LEVEL_ORDER[a] >= LEVEL_ORDER[b] ? a : b) as Risk;
}
...
export interface RiskClassification {
  level: Risk;
  riskyFiles: Array<{ path: string; risk: Risk; reason: string }>;
}

/** Classify risk from an array of file paths. Returns { level, riskyFiles }. */
export function classifyRisk(paths: string[]): RiskClassification {
  if (!paths || paths.length === 0) {
    return { level: 'low', riskyFiles: [] };
  }

  let highestLevel: Risk = 'low';
  const riskyFiles: Array<{ path: string; risk: Risk; reason: string }> = [];
...
export function estimateComplexity({ prompt, fileCount = 0, risk = 'low', intent = 'edit', priorFailures = 0 }: {
  prompt: string;
  fileCount?: number;
  risk?: string;
  intent?: string;
...
export function inferTier({ intent, risk, complexity, effort, specialistTierBias }: {
  intent: string;
  risk: string;
  complexity: string;
  effort: string;
  specialistTierBias?: string | null;
}): Tier {
...
function requiresWrite(intent: string): boolean {
...
function buildExplanation({ intent, risk, complexity, fileCount, priorFailures }: {
  ...
  const complexityWord = ({ trivial: 'Trivial', simple: 'Simple', moderate: 'Moderate', complex: 'Complex' } as Record<string, string>)[complexity];
 ...
 */
export function classifyReasoningDepth(prompt: string, files: string[] = [], priorOutcomes: PriorOutcome[] = []): ReasoningDepthResult {
...
const PLUGIN_KEYWORD_MAP: Record<string, string> = {
  stripe: 'stripe', payment: 'stripe', checkout: 'stripe', subscription: 'stripe', webhook: 'stripe',
  slack: 'slack', teams: 'teams',
  supabase: 'supabase', neondb: 'neon-postgres',
  github: 'github', 'pull request': 'github', linear: 'linear',
  jira: 'atlassian-rovo', atlassian: 'atlassian-rovo',
  gmail: 'gmail', outlook: 'outlook-email', notion: 'notion',
  'google calendar': 'google-calendar', 'google drive': 'google-drive',
  sentry: 'sentry', vercel: 'vercel', netlify: 'netlify', cloudflare: 'cloudflare',
  amplitude: 'amplitude', figma: 'figma', canva: 'canva',
  hubspot: 'hubspot', pipedrive: 'pipedrive',
  sendgrid: 'sendgrid', twilio: 'twilio-developer-kit',
  sharepoint: 'sharepoint', box: 'box',
  openai: 'openai-developers', 'hugging face': 'hugging-face',
  razorpay: 'razorpay', render: 'render',
  monday: 'monday-com', asana: 'asana', clickup: 'clickup',
};

/**
 * Detect Codex plugin IDs that match keywords in the prompt.
 */
export function detectSuggestedPlugins(prompt: string): string[] {
  if (!prompt) return [];
...
interface CIRiskResult {
  hasCIFailure: boolean;
  failedBranch: string | null;
}

function checkCIRisk(cwd?: string): CIRiskResult {
  try {
    ...
    const runs: Array<{ conclusion: string; headBranch: string }> = JSON.parse(json);
...
interface SpecialistDef {
  triggers: { extensions?: string[]; keywords?: string[] };
  tier_bias?: string;
  prompt_file?: string;
}

const DEFAULT_SPECIALISTS: Record<string, SpecialistDef> = {
...
function loadSpecialistRegistry(): Record<string, SpecialistDef> {
  try {
 ...
 */
export function classifySpecialist(prompt = '', files: string[] = []): SpecialistResult {
  const promptLower = prompt.toLowerCase();
  const scores: Record<string, number> = {};
  const matchedTriggers: Record<string, string[]> = {};
  ...
  const confidence: 'medium' | 'high' = bestScore >= 4 ? 'high' : 'medium';
  return { specialist: best, confidence, triggers: matchedTriggers[best] };
}

// ─── Agent registry bridge (synchronous, injected) ───────────────────────────

type MatchAgentFn = (intent: string, risk: string, taskType: string) => unknown[];

let _matchAgentFn: MatchAgentFn | null = null;

/**
 * Prime the agent registry so detectTask can match agents synchronously.
 */
export async function primeAgentRegistry(): Promise<void> {
  try {
    // @ts-ignore — registry.mjs not yet migrated
    const { matchAgent } = await import('./agents/registry.js');
    _matchAgentFn = matchAgent as MatchAgentFn;
  } catch {
    // Registry unavailable — detectTask continues without agent matching
  }
}

function _matchAgentSync(intent: string, risk: string, taskType: string): unknown | null {
  try {
...
interface DetectTaskInput {
  prompt?: string;
  files?: string[];
  priorFailures?: number;
  sessionContext?: {
    priorAttempts?: Array<{ failed?: boolean; status?: string }>;
    riskSignals?: Array<{ type?: string; failed?: boolean } | string>;
  } | null;
  headJudgment?: {
    depth?: string;
    situation?: {
      taskShape?: { risk?: Risk; ambiguity?: string };
    };
  } | null;
  cwd?: string;
}
...
export function detectTask(input: DetectTaskInput): Record<string, unknown> {
  ...
  if (sessionContext) {
    ...
    if (riskSignals.some(s => s && (typeof s === 'object' && ((s as { type?: string }).type === 'failure' || (s as { failed?: boolean }).failed)) || (typeof s === 'string' && /fail/i.test(s)))) {
  ...
  let risk: Risk = higherRisk(pathRiskLevel, keywordRisk);
  ...
  function bumpEffort(e: string, n = 1): string {
    return effortOrder[Math.min(effortOrder.indexOf(e) + n, effortOrder.length - 1)];
  }

  let effort: string = ({ trivial: 'low', simple: 'medium', moderate: 'high', complex: 'high' } as Record<string, string>)[complexity];
  ...
  // 9. Reasoning depth
  const priorOutcomes: PriorOutcome[] = priorFailures > 0
    ? Array.from({ length: priorFailures }, () => ({ failed: true }))
    : [];
  let { depth: reasoningDepth, signals: reasoningSignals } = classifyReasoningDepth(prompt, files, priorOutcomes);

  // 10. Suggested Codex plugins
  const suggestedPlugins = detectSuggestedPlugins(prompt);

  // 11. CI risk
  const ciRiskResult = checkCIRisk(input.cwd || process.cwd());

  // 12. Match specialized agent
  const suggestedAgent = _matchAgentSync(intent, risk, specialistResult.specialist || '');

  // HEAD judgment override
  const headOverrides: Record<string, string> = {};
  if (headJudgment?.situation) {
    ...
    const headDepthMap: Record<string, 'low' | 'medium' | 'high' | 'ultra'> = { reflexive: 'low', light: 'medium', full: 'high', deep: 'ultra' };
    const headDepth = headJudgment.depth ? headDepthMap[headJudgment.depth] : undefined;
    if (headDepth) {
      const depthOrder: Record<string, number> = { low: 0, medium: 1, high: 2, ultra: 3 };
...
if (process.argv[1] && new URL(import.meta.url).pathname === process.argv[1]) {
  ...
  if (!prompt) {
    console.error('Usage: node src/detect.ts "task description" [--files a,b]');
    process.exit(1);
  }
