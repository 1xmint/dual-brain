 ...
 */
export function scorePrompt(prompt: string, opts: { type?: string; maxTokenBudget?: number } = {}): Record<string, unknown> {
 ...
 */
export function logPromptExchange(exchange: Record<string, unknown>): Record<string, unknown> {
  const {
    type = 'think',
    round = 1,
    prompt,
  ...
  const entry = {
    ...
    issues: promptScore?.issues?.map((i: any) => i.rule) || [],
  };
 ...
 */
export function getPromptStats(opts: { days?: number } = {}): Record<string, unknown> {
  const { days = 7 } = opts;
  const logFile = join(AUDIT_DIR, 'exchanges.jsonl');
  ...
  const topIssues = Object.entries(commonIssues)
    ...
    .slice(0, 5)
    .map(([rule, count]) => ({ rule, count, pct: Math.round((count as number) / entries.length * 100) }));
 ...
 */
export function suggestImprovements(prompt: string, type = 'think'): Record<string, unknown> {
  const score = scorePrompt(prompt, { type });
  const suggestions = [];

  for (const issue of score.issues as Array<{ rule: string; msg: string; penalty: number }>) {
    switch (issue.rule) {
      case 'too-short':
