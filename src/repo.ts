...
function detectNode(cwd: string) {
  ...
  const monorepo = Boolean(
    (pkg as Record<string, unknown>).workspaces ||
  ...
  return {
    type: 'node',
    name: (pkg as Record<string, unknown>).name || null,
    packageManager,
...
function detectRust(cwd: string) {
...
function detectPython(cwd: string) {
...
function detectRuby(cwd: string) {
 ...
 */
export function getFileOwnership(filePath: string, cwd: string) {
  try {
 ...
 */
export function getDependencyEdges(filePath: string, cwd?: string) {
  try {
 ...
 * @returns {string[]}
 */
export function findRelatedTests(filePath: string, cwd?: string) {
 ...
 */
export function getRiskHotspots(cwd?: string, opts: { days?: number; limit?: number } = {}) {
...
function detectPrimaryLanguage(cwd: string) {
  try {
 ...
 * @returns {object}
 */
export function getRepoIntelligence(cwd?: string) {
  const root = cwd || process.cwd();
